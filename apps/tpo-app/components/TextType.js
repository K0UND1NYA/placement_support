...
const TextType = ({
    ...
    const resolvedVariableSpeed = useMemo(() => {
        ...
        if (variableSpeedEnabled) return { min: variableSpeedMin ?? 20, max: variableSpeedMax ?? 120 };
        return null;
    ...
    useEffect(() => {
        if (showCursor && cursorRef.current) {
            try {
                ...
                // gsap may not be available or run on server; ignore errors
            }
        }
