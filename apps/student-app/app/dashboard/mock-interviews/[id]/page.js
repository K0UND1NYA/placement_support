  ...
  useEffect(() => {
      ...
      const logViolation = async (type, details = {}) => {
          try {
              ...
              console.warn(`Integrity Warning: ${type}`);
              
              await apiFetch('/mock-interview/log-integrity', {
                  method: 'POST',
                  body: JSON.stringify({
      ...
      const handleVisibilityChange = () => {
          if (document.hidden) {
              logViolation('tab_switch', { timestamp: new Date() });
              alert("Warning: Tab switching is monitored and recorded!");
          }
      };

      const handleBlur = () => {
          // Window lost focus (alt-tab, another app)
           logViolation('window_blur', { timestamp: new Date() });
      };

      const handleCopy = (e) => {
          // e.preventDefault(); // Optional: block it entirely? User asked to "monitor".
          logViolation('copy_attempt', { timestamp: new Date() });
      };

      const handlePaste = (e) => {
          logViolation('paste_attempt', { timestamp: new Date() });
      };
