...
exports.startOTPCleanupScheduler = startOTPCleanupScheduler;
...
async function cleanupExpiredOTPs() {
    try {
        const result = await (0, db_1.query)(`DELETE FROM login_otps 
...
function startOTPCleanupScheduler() {
    ...
    cleanupExpiredOTPs().catch(err => console.error('[OTP Cleanup] Initial cleanup failed:', err));
    // Then run every 12 hours (12 * 60 * 60 * 1000 ms)
    const TWELVE_HOURS = 12 * 60 * 60 * 1000;
    setInterval(() => {
        cleanupExpiredOTPs().catch(err => console.error('[OTP Cleanup] Scheduled cleanup failed:', err));
    }, TWELVE_HOURS);
