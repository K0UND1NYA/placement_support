...
router.get('/:examId/questions', auth_1.authenticateJWT, isolation_1.collegeIsolation, async (req, res) => {
    const { examId } = req.params;
    try {
        // Verify exam belongs to user's college
        if (req.user?.role !== 'admin') {
            const check = await (0, db_1.query)('SELECT id FROM exams WHERE id = $1 AND college_id = $2', [examId, req.user?.college_id]);
            if (check.rows.length === 0)
                return res.status(403).json({ error: 'Unauthorized' });
        }
        const result = await (0, db_1.query)('SELECT id, question, options FROM questions WHERE exam_id = $1', [examId]);
        res.json(result.rows);
    }
    catch (err) {
...
router.post('/', auth_1.authenticateJWT, (0, auth_1.authorizeRoles)('tpo'), async (req, res) => {
    ...
    try {
        const result = await (0, db_1.query)('INSERT INTO exams (title, code, duration, college_id, created_by, start_time, end_time) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *', [title, code, duration, req.user?.college_id, req.user?.id, start_time || null, end_time || null]);
...
router.post('/:examId/questions', auth_1.authenticateJWT, (0, auth_1.authorizeRoles)('tpo'), async (req, res) => {
    const { examId } = req.params;
    const { questions } = req.body; // Array of { question, options, correct_answer }
    try {
        // Verify exam belongs to the TPO's college
        const examCheck = await (0, db_1.query)('SELECT id FROM exams WHERE id = $1 AND college_id = $2', [examId, req.user?.college_id]);
        ...
        for (const q of questions) {
            await (0, db_1.query)('INSERT INTO questions (exam_id, question, options, correct_answer) VALUES ($1, $2, $3, $4)', [examId, q.question, JSON.stringify(q.options), q.correct_answer]);
        }
...
router.post('/:examId/attempt', auth_1.authenticateJWT, (0, auth_1.authorizeRoles)('student'), async (req, res) => {
    const { examId } = req.params;
    ...
    try {
        ...
        const questions = await (0, db_1.query)('SELECT id, correct_answer FROM questions WHERE exam_id = $1', [examId]);
        let score = 0;
        ...
        const attempt = await (0, db_1.query)('UPDATE attempts SET score = $1, submitted_at = NOW() WHERE id = $2 AND student_id = $3 AND submitted_at IS NULL RETURNING *', [score, attempt_id, req.user?.id]);
...
router.get('/:examId/integrity', auth_1.authenticateJWT, (0, auth_1.authorizeRoles)('tpo', 'admin'), async (req, res) => {
    const { examId } = req.params;
    try {
        ...
        const logs = await (0, db_1.query)(`SELECT il.*, a.student_id, u.name as student_name, u.email as student_email 
             FROM integrity_logs il
             JOIN attempts a ON il.attempt_id = a.id
             JOIN users u ON a.student_id = u.id
             WHERE a.exam_id = $1
             ORDER BY il.created_at DESC`, [examId]);
        res.json(logs.rows);
    }
    catch (err) {
...
router.delete('/:examId', auth_1.authenticateJWT, (0, auth_1.authorizeRoles)('tpo', 'admin'), async (req, res) => {
    const { examId } = req.params;
    try {
        // Verify exam belongs to college
        if (req.user?.role !== 'admin') {
        ...
        // 5. exams
        await (0, db_1.query)('DELETE FROM integrity_logs WHERE attempt_id IN (SELECT id FROM attempts WHERE exam_id = $1)', [examId]);
        await (0, db_1.query)('DELETE FROM attempts WHERE exam_id = $1', [examId]);
        await (0, db_1.query)('DELETE FROM applications WHERE exam_id = $1', [examId]);
        // questions has ON DELETE CASCADE in schema, but we can be explicit
        await (0, db_1.query)('DELETE FROM questions WHERE exam_id = $1', [examId]);
        await (0, db_1.query)('DELETE FROM exams WHERE id = $1', [examId]);
